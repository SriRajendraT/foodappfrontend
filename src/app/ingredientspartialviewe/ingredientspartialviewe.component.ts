...
export class IngredientspartialvieweComponent implements OnInit {
  ...
  removeInput(id:number=0,index:number=-1) {
    // let vname = new Ingredient();
    // vname.INGREDIENT_NAME = 'Rj' + i;id
    // vname.QUANTITY = i + 'kg';
    debugger;
    ...
    if (id > 0) {
      kv.value = id;
      this.dataapi
        .deleteIngredientById(kv)
        .subscribe((result: ApiResponse<boolean>) => {
          if (result) {
            this.ingredints=this.ingredints.filter(e=>e.ID!=id);
            
          }
        });
    } else {
      this.ingredints=this.ingredints.filter((i,e)=>{
        console.log(e,i);
